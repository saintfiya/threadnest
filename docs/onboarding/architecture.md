...
```text
公开：/、/static/*、/ping、/swagger/*、/api/v1/signup、/api/v1/login、/api/v1/posts
...
键统一由 `dao/redis/keys.go: getRedisKey` 添加 `goweb:` 前缀：

| Key | 类型 | member | score | 写入点 |
| --- | --- | --- | --- | --- |
| `goweb:post:time` | ZSET | post ID | Unix 发布时间 | `redis.CreatePost` |
| `goweb:post:score` | ZSET | post ID | 初始发布时间 + 投票增量 | `redis.CreatePost` / `redis.VoteForPost` |
| `goweb:post:voted:<postID>` | ZSET | user ID | `-1` 或 `1` | `redis.VoteForPost` |
