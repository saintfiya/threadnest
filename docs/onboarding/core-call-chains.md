...
基础列表链为 `GET /api/v1/posts → controller.PostList → getPageInfo → logic.PostList → mysql.PostList → ResponseSuccess`，固定按 MySQL `create_time DESC` 返回部分帖子字段。

增强列表链为 `GET /api/v1/posts2 → GetPostListHandler2 → logic.GetPostList2`：无社区过滤时从 Redis 时间榜或分数榜直接分页取 ID；指定 `community_id` 时先从 MySQL 取候选 ID，再读取 Redis 分数、排序并分页；随后 `mysql.GetPostListByIDs` 用三次批量查询补齐帖子、作者和社区，最后 `redis.GetPostVoteData` 计算赞成票减反对票。参数默认 `page=1,size=10,order=time`，页大小上限为 100。
