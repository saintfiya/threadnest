...
- `docs/swagger.yaml`：当前八个业务路由的生成契约；仍以 `router.SetupRouter` 验证实际注册顺序。
...
- `goweb_log`：历史运行时日志，可能含旧版本记录的敏感请求头，不应作为常规学习材料；新日志已有脱敏保护。
...
| 练习 | 比较 `models.ParamPostList`、Swagger 和实际 handler；定位尚未接入的社区/排序参数 |
